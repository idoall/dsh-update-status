	...
	factory: (require) => {
		...
		*/
		...
		var StatusStore = class {
			...
			async refresh(channel = this.snapshot.status?.channel ?? "latest") {
			...
			request(force, channel = this.snapshot.status?.channel ?? "latest") {
		...
		var PreferencesStore = class {
			...
			publish(next) {
				const previous = this.snapshot;
				if (previous.sidebarEnabled === next.sidebarEnabled && previous.channel === next.channel && previous.writable === next.writable && previous.status === next.status) return;
				this.snapshot = next;
				for (const listener of this.listeners) listener();
			}
		};
...
.dus-brand-deepseek{color:var(--dsw-alias-text-primary,var(--dsw-alias-text-l1,#f4f4f5));font-size:15px;font-weight:650;letter-spacing:-.015em;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.dus-badge{align-items:center;gap:5px;background:color-mix(in srgb,var(--dsw-alias-button-floating-fill,#222) 82%,transparent);border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.16));border-radius:999px;color:var(--dsw-alias-text-secondary,#bbb);cursor:pointer;display:inline-flex;flex:0 0 auto;font-size:11px;font-variant-numeric:tabular-nums;line-height:20px;max-width:132px;outline:none;padding:0 8px;touch-action:manipulation;user-select:none}
